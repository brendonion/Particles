...
namespace window {
	...
	void Screen::setPixel(int x, int y, Uint8 red, Uint8 green, Uint8 blue) {
		...
		// Move down the screen y times SCREEN_WIDTH, then add x to the position
	...
	void Screen::boxBlur() {
		...
		for (int y = 0; y < SCREEN_HEIGHT; y++) {
			for (int x = 0; x < SCREEN_WIDTH; x++) {
				// Iterate through this grid of 9 pixels to get average color values of surrounding pixels
				/*
				 * 0 0 0
				 * 0 1 0 
				 * 0 0 0
	...
	void Screen::close() {
		// Delete buffer and destroy window 
		delete []m_buffer1;
		delete []m_buffer2;
		SDL_DestroyRenderer(m_renderer);
